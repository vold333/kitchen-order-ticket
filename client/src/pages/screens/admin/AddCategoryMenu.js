        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl p-6 space-y-4 relative">
            ...
            <input
              ...
              onChange={(e) => {
                const value = e.target.value.slice(0, 8); // Limit to 10 digits
        ...
        {filteredItems.map((item) => (
          <Paper elevation={4} sx={{borderRadius: "20px"}}
            ...
            {item.image ? (
              ...
              <div className="w-full h-32 sm:h-36 md:h-28 lg:h-36 xl:h-48 rounded-xl bg-gray-300 flex items-center justify-center text-gray-600 object-cover mb-1 sm:mb-2">
                No Image
              </div>
            )}
